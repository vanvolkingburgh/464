...
CREATE TABLE IF NOT EXISTS `amazongo`.`CustomerFeedback` (
  ...
  `FeedbackNotes` VARCHAR(100) NULL,
  `CustomerID` VARCHAR(36) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `amazongo`.`Receipt` (
  ...
  `ItemTotal` VARCHAR(45) DEFAULT NULL,
  `ItemTaxAmount` VARCHAR(45) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `amazongo`.`FeedbackResponse` (
  ...
  `ResponseNotes` VARCHAR(100) NULL,
  `FeedbackLogID` INT NOT NULL,
  `EmployeeID` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `amazongo`.`Suppliers` (
  ...
  `StoreID` VARCHAR(36) NOT NULL,
  `ItemID` INT NOT NULL,
  `ItemPriceofSupplier` DECIMAL(10,2) NOT NULL,
  `DiscountThreshold` INT DEFAULT 200,
  `DiscountedPriceofSupplier` DECIMAL(10,2) NOT NULL,
  PRIMARY KEY (`SupplierID`),
  INDEX `fk_Suppliers_Store1_idx` (`StoreID` ASC) VISIBLE,
  INDEX `fk_Suppliers_Item1_idx` (`ItemID` ASC) VISIBLE,
  CONSTRAINT `fk_Suppliers_Store1`
    FOREIGN KEY (`StoreID`)
    REFERENCES `amazongo`.`Store` (`StoreID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `amazongo`.`Inventory` (
  ...
  UNIQUE INDEX `inventory_unique` (`InventoryID`, `ItemID`, `StoreID` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `amazongo`.`RestockOrder` (
  ...
  INDEX `fk_RestockOrder_Inventory1_idx` (`InventoryID`, `ItemID`, `StoreID` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_RestockOrder_Inventory1`
    FOREIGN KEY (`InventoryID`, `ItemID`, `StoreID`)
    REFERENCES `amazongo`.`Inventory` (`InventoryID`, `ItemID`, `StoreID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `amazongo`.`FraudResolution` (
  ...
  INDEX `fk_FraudResolution_FraudDetectionLog1_idx` (`FraudDetectionLogID`, `TransactionID` ASC) VISIBLE,
  INDEX `fk_FraudResolution_Employees1_idx` (`EmployeeID` ASC) VISIBLE,
  CONSTRAINT `fk_FraudResolution_FraudDetectionLog1`
    FOREIGN KEY (`FraudDetectionLogID`, `TransactionID`)
    REFERENCES `amazongo`.`FraudDetectionLog` (`FraudDetectionLogID`, `TransactionID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
